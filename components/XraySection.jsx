...
function XraySection() {
  ...
  useEffect(() => {
    ...
    gsap.to(".xray-image", { // The scroll trigger starts when we hit this image in the build.
      scrollTrigger: {
        trigger: ".xray-image",
        scrub: true,
        start: "bottom bottom",
        onUpdate: (self) => {
          ...
          } // We split our scroll zone into three regions here. Below 80%, between 80 and 100%, and 100% to show three different Airpod Images.
        },
      },
      scale: 0.8,
      duration: 2,
  ...
  return (
    <div id="XraySection" className="w-full  h-[350vh] transition-opacity duration-700  flex  relative " >
      <video loop autoPlay muted src={NoiseVideo} className="w-full h-full fixed top-0 opacity-0 noise-video z-[-1] lg:object-scale-down object-cover"></video>
      ...
      <div className="h-screen sticky top-0 w-full flex justify-center">
        <div className="w-[90%]  flex  z-10 sticky top-0 items-center justify-center ">
          <Image src={xrayImage} alt="xray image" className="h-80% xray-image z-10 " />
          {/* The scroll trigger starts when we hit this image */}
        </div>
      </div>
